...
Module   : Financeiro
Source   : WFON001
Objective: Relatório de Lembrete de Vencimento (boleto)
*///#########################################################################################
...
User Function WFON001()
	Prepare Environment Empresa "01" Filial "01"
		QOUT("##### INICIANDO PROCESSAMENTO RELATORIO LEMBRETE DE VENCIMENTO #####")
    	U_WFON001C()
		QOUT("##### FINALIZANDO PROCESSAMENTO RELATORIO LEMBRETE DE VENCIMENTO #####")
	Reset Environment
Return

User Function WFON001C()
	Local   cCabec  := ""
	Local   aItens  := {}
	Local   nTotal  := 0
	Local   cBody   := ""
	...
	cQuery := "SELECT "
	cQuery += " A1_COD, A1_LOJA, A1_NOME, A1_EMAIL, A1_YDIALEM, A1_VEND, A3_EMAIL "
	cQuery += "FROM "+RETSQLNAME("SA1")+" SA1 "
	cQuery += " INNER JOIN "+RETSQLNAME("SA3")+" SA3 ON "
	cQuery += "  A1_FILIAL=A3_FILIAL AND A1_VEND=A3_COD "
	cQuery += "WHERE "
	cQuery += " SA1.D_E_L_E_T_ != '*' AND SA3.D_E_L_E_T_ != '*' AND A1_YDIALEM != '' AND "
	cQuery += " A1_EMAIL!='' AND A1_EST!='EX' "

	TCQUERY cQuery NEW ALIAS T01
	DBSELECTAREA("T01")

	cDiaLem  := StrTokArr(Alltrim(T01->A1_YDIALEM),"-")
	cLemQry  := ""
	For nReg :=1 to Len(cDiaLem)
		cLemQry += "'"+DtoS(date()+val(cDiaLem[nReg]))+"'"
		If ( nReg+1 ) <= Len(cDiaLem)
			cLemQry += "," 
		Endif
	Next nReg
	...
	WHILE !T01->(EOF())
		...
		//cEmailV := IIF(!Empty(T01->A3_EMAIL),Alltrim(T01->A3_EMAIL),"") //Utilizar e-mail do vendedor para cobrança
		cEmailV := ""
		...
		//cQuery += " E1_VENCREA IN ("+cLemQry+") AND E1_SALDO > 0 AND "
		cQuery += " E1_VENCREA IN ("+cLemQry+") AND E1_SALDO>0 AND E1_NATUREZ='101001' AND "
		...
		If Len(aItens) > 0
			...
			For i:=1 to Len(aItens)
				...
				//If !Empty(aItens[i][2]) // Título (soma)
				If !Empty(aItens[i][1]) // Título (soma)
					...
					cItens += " <td align='right'  width='12%' bgcolor='"+ALLTRIM(cCor)+"'><font size='2' face='Arial'>" + TransForm(aItens[i][6],"@E 99,999,999.99")+"</td>"
				Else
					...
					cItens += " <td align='right'  width='12%' bgcolor='"+ALLTRIM(cCor)+"'><font size='2' face='Arial'><b>" + TransForm(aItens[i][6],"@E 99,999,999.99")+"</b></td>"
				EndIf
				cItens += "</tr>"
			...
			//cCC      := ""
			cBCC     := GETMV("MV_YCBCC")
			cAssunto := "Lembrete de Vencimento - FONNET NETWORKS"
			cBody := "<p align=left><font face=Calibri size=4 color=#000000>"
			cBody += "<br>Prezado(a) cliente, "+ALLTRIM(T01->A1_NOME)
			cBody += "<br>"
			cBody += "<br>Estou entrando em contato para lembrá-lo(a) que o vencimento do seu boleto está chegando."
			cBody += "<br>"
			cBody += cBodyt
			cBody += "<br>Se tiver alguma dúvida ou quiser saber sobre seus próximos vencimentos, é só entrar em contato."
			cBody += "<br>"
			cBody += "<br>Conte sempre conosco!"
			cBody += "<br>"
...
Static Function MontaXML(colunas, dados)
	...
	Next x
	oExcel:Activate()
	oExcel:GetXMLFile("\DATA\Lembrete.xls")
Return

/* 26/08/2021 - Brena solicitou a retirada do arquivo*/
//Static Function TEnvMail(cPara,cAssunto,cMensagem,cArquivo,cCC)

USER FUNCTION EMAILFIN()
cPara := "erlandeson.gomes@fonnet.com.br"
cAssunto := "teste"
cMensagem := "ola mundo"
cCC := ""
cBCC := ""
//tstsmtp("smtp.office365.com", "587",, "erlandeson007sec@outlook.com", "bone.0017", SuperGetMv("MV_RELAUTH",,.F.), 120)
TEnvMail(cPara,cAssunto,cMensagem,cCC,cBCC)
RETURN
...
Static Function TEnvMail(cPara,cAssunto,cMensagem,cCC,cBCC)
	...
	oMessage:Clear()
	oMessage:cFrom   := "erlandeson007sec@outlook.com"
	...
	*/
	...
	oServer:SetUseSSL(.T.)
	*/
	xRet := oServer:Init( "", "smtp.office365.com", "erlandeson007sec@outlook.com", "bone.0017", 0, nPorta )
	If xRet != 0
	...
	If lMailAuth
		xRet := oServer:SmtpAuth( "erlandeson007sec@outlook.com", "bone.0017" )
		If xRet != 0
