...
User Function PrtNfeSef(cIdEnt, cVal1		, cVal2		, oDanfe,;
						oSetup, cFilePrint	, lIsLoja	, nTipo	,;
 						aRECNO, cPath, cNome, cPathServe, lBol, nCont, cMvpar01)
...
Default cNome  := cFilePrint
Default cPathServe := cDirTemp
Default nCont  := 1
Default cMvpar01  := ""
...
//cPathPDF	:= cPath
cNomePDF	:= cNome  
cGravaPDF   := oSetup:aOptions[6]:= cPath

IF len(aSf2RECNO) > 0 .and. !lBol 	
	lRelAuto	:= .T.
Else
  //volto o MV_PAR01 paro o selecionado no oSetup pq ele se altera assumindo o MV_PAR02
  //sivaldo 20170901
	If nCont > 1
		MV_PAR01 := cMvpar01
	EndIF
...
IF lRelAuto      
	...
	lDisabeSetup		:= .T.
	lTReport			:= NIL                                                                                                                        
//	oSetup		 		:= FWPrintSetup():New(PD_ISTOTVSPRINTER+PD_DISABLEDESTINATION, "Set Device Printer")
	cPrinter			:= "COMPILAPDF"
	lServer				:= .f.
	lPDFAsPNG			:= NIL
	lRaw				:= NIL
	...
	//oDanfe	:=	FWMsPrinter():New( cFileName, IMP_PDF, lToLegacy, cPathSrv, lDisabeSetup, lTReport, @oSetup, cPrinter,lServer, lPDFAsPNG, lRaw, lViewPDF, nQtdCopy )
	     
	//oDanfe:nDevice		:= nDevice
    oDanfe:cPathPDF	:= cPath
	oDanfe:cPrinter	:= cPrinter 
...
If nTipo <> 1
	...
	Else
		...
		If lVerPerg .and. !lRelAuto .and. len(aSf2RECNO) <= 0 .and. !lBol
			
		//	ParamBox(aPerg,"SPED - NFe",@aParam,,,,,,,cParNfeExp,.T.,.T.)
	...
	If lPergunte  .and. lRelAuto
		RPTStatus( {|lEnd| DANFEProc(@oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja)}, "Gerando E-mail..." )		
	Else
	...
	IF lRelAuto
		...
		//³ Adiciona Boletos a Nota ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		
		
		//| Necessário Setar o parametro para TRUE para compatibilizar o boleto|
		//| Jonatas Oliveira - Compila - 23/06/2016							   |	
		oDanfe:LADJUSTTOLEGACY := .T.	
			
		SF2->(DBGOTO(aSf2RECNO[1]))
		...
		oDanfe:print()//Grava arquivo e Visualiza antes de imprimir se o lViewPDF for  == .T.
		
		
		IF FILE(cDirTemp + cNomePDF+'.pdf')	
			aRet	:= { .T.,cDirTemp + cNomePDF,"2",cPathPDF}			
		ELSEIF FILE(cPathPDF + cNomePDF+'.pdf')
			aRet	:= { .T.,cPathPDF + cNomePDF+'.pdf',"2",cPathPDF}		
		ELSEIF FILE(cGravaPDF + cNomePDF+'.pdf')
		    aRet	:= { .T.,cGravaPDF + cNomePDF+'.pdf',"2",cPathPDF}	
		Else
	       aRet	:= {.F.,"","3",cPathPDF} 
		ENDIF
				...
				If !Empty(aXML[nX][2])
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						IF SE1->(DBSEEK(SF2->(F2_FILIAL + F2_CLIENTE + F2_LOJA + F2_SERIE + F2_DOC )))
							...
							ENDDO
							//If lRelAuto	.or. 		
								U_RFIN025(, @oDanfe,  aRecSE1)
					        //EndIF
						ENDIF	
						
						oDanfe:LADJUSTTOLEGACY := .F.
