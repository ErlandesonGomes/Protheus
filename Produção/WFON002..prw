...
Module   : Financeiro
Source   : WFON002
Objective: Relatório de Aviso de Cobrança (boleto)
*///#########################################################################################
...
User Function WFON002C()
	...
	WHILE !T01->(EOF())
		...
		cQuery += " E1_VENCREA IN ("+cCobQry+") AND E1_SALDO > 0 AND E1_NATUREZ='101001' AND "
		...
		If Len(aItens) > 0
			...
			cBody += "<br>"
			cBody += "<br>Até o momento, não identificamos em nossos sistemas o pagamento do(s) título(s) abaixo:"
			cBody += "<br>"
			cBody += cBodyt
